...
def parse(String description) {
    ...
    def json_obj = new groovy.json.JsonSlurper().parseText(bodyString)
 
	if (bodyString) {
        log.debug "Parsing: $bodyString"
//    	def parts = bodyString.split(" ")
    	def name  = json_obj.type+json_obj.uid//parts.length>0?parts[0].trim():null
    	def value = json_obj.state//parts.length>1?parts[1].trim():null
        
		//def nameparts = name.split("\\d+", 2)
		def namebase = json_obj.type//nameparts.length>0?nameparts[0].trim():null
        def namenum = String.valueOf(json_obj.uid)//name.substring(namebase.length()).trim()
...
void childOn(String dni) {
	def uid = dni.split("-")[-1]
    def nameparts = uid.split("\\d+", 2)
	def namebase = nameparts.length>0?nameparts[0].trim():null
    uid = uid.substring(namebase.length()).trim()
    log.debug "childOn($uid)"
    sendEthernet("{\"uid\": ${uid}, \"cmd\": \"on\"}")
}
...
void childOff(String dni) {
	...
	def namebase = nameparts.length>0?nameparts[0].trim():null
    uid = uid.substring(namebase.length()).trim()
    log.debug "childOff($uid)"
    sendEthernet("{\"uid\": ${uid}, \"cmd\": \"off\"}")
}
